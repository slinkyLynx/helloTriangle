...
private:
	void initWindow() {
		...
		mWindow = glfwCreateWindow(mWidth, mHeight, "Vulkan test", nullptr, nullptr);
	}
	
	bool checkExtensions(const uint32_t glfwExtensionCount, const char** glfwExtensions) {
		...
		for (uint32_t i{0u}; i < glfwExtensionCount; ++i) {
			for (auto extension : extensions) {
				if (static_cast<std::string>(glfwExtensions[i]) == static_cast<std::string>(extension.extensionName)) {
					++foundExtensions;
					break;
				}
			}
